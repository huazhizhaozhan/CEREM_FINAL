...
from modelv21_2 import UIE, convert_example
...
parser.add_argument('--device_list', default=[0, 1, 2, 3], type=list, help="Select which device to train model, defaults to gpu.")
...
def train():
    ...
    # print(model)
    model_u = model.to(args.device)
    model_u.freeze(flag=True)
    model = DataParallel(model_u, device_ids=args.device_list)
    tokenizer = AutoTokenizer.from_pretrained("pretrain/chinese-roberta-wwm-ext")                    # 加载tokenizer，ERNIE 3.0
    ...
    # 最多保存5个过程模型，保存最新的5个，如果超过了，就把之前的删掉
    now_save_mums = 0
    freeze_flag = True
    
    for epoch in range(1, args.num_train_epochs+1):
        # model_u.freeze(flag = False)
        if best_f1 > 0.2 and freeze_flag == True:
            freeze_flag = False
            model_u.freeze(flag = False)
            cur_save_dir = os.path.join(args.save_dir, "model_unfreeze")
            if not os.path.exists(cur_save_dir):
                os.makedirs(cur_save_dir)
            torch.save(model, os.path.join(cur_save_dir, 'model.pt'))
            tokenizer.save_pretrained(cur_save_dir)

        # print(model.freeze_f)
        for batch in train_dataloader:
            ...
            # print('这个是真实值，我要看看有没有绕开prompt',start_ids, end_ids)
            ...
            if global_step % args.logging_steps == 0:
                ...
                str_log = "global step %d, epoch: %d, loss: %.5f, speed: %.2f step/s"% (global_step, epoch, loss_avg, args.logging_steps / time_diff)
                with open(args.txt_log_name, 'a', encoding='utf-8') as file:
                    # 逐行写入文本
                    file.write(str_log+"\n")
            ...
            if global_step % args.valid_steps == 0:
                ...
                now_save_mums += 1
                if now_save_mums > 5:
                    rm_name = global_step-5*args.valid_steps
                    rm_folder_path = os.path.join(args.save_dir, "model_%d" % rm_name)
                ...
                str_log = "Evaluation precision: %.5f, recall: %.5f, F1: %.5f" % (precision, recall, f1)
                with open(args.txt_log_name, 'a', encoding='utf-8') as file:
                    # 逐行写入文本
                    file.write(str_log+"\n")
                if f1 > best_f1:
                    ...
                    str_log = f"best F1 performence has been updated: {best_f1:.5f} --> {f1:.5f}"
                    with open(args.txt_log_name, 'a', encoding='utf-8') as file:
                        # 逐行写入文本
                        file.write(str_log+"\n")
                    best_f1 = f1
        ...
        if args.auto_da_epoch > 0 and epoch % args.auto_da_epoch == 0:
            ...
            model = torch.load(os.path.join("pretrain/chinese-roberta-wwm-ext", 'pytorch_model.bin'))        # 重新加载预训练模型
